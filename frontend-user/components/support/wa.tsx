 ...
 *
 * Duplicated from frontend-admin/components/support/wa.tsx on purpose: the
 * two frontends are independently deployed Next apps with no shared package,
 * so a `components/` copy is cheaper than standing up workspace tooling for
 * one file. Keep the two in sync when either changes.
 *
    ...
    <div className={cn("flex px-[3%] py-[1px]", mine ? "justify-end" : "justify-start")}>
      ...
      >
        ...
        {m.attachment_url && hasAudio && (
          // Voice note — inline player, not a download link.
          <audio
            controls
            preload="metadata"
