 ...
 * instead of the super-admin "MarginPlant" default. On the platform host it
 * returns the MarginPlant defaults; on a branded domain it fetches the
...
async function resolveHostBrand(): Promise<{
  ...
  if (platform || !host) {
    return { platform: true, name: "MarginPlant Broker", logo: null };
  }
  try {
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const displayName = name || (platform ? "MarginPlant Broker" : "Trading Platform");
  const title = platform
    ? "MarginPlant Broker — Indian Trading Platform"
  ...
  return {
    ...
    // shares with MarginPlant's name + logo instead of the admin's".
    openGraph: {
      type: "website",
